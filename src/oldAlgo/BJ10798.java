...
public class BJ10798 {
    public static void main(String[] args) throws IOException {
        ...
        // 입력은 총 5줄이고
        // 한 줄에 최대 15글자 이고 빈칸없이 연속으로 받는다
        // Scanner sc = new Scanner(System.in);
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));

        // char 타입의 2차원 배열의 초기값은 '/0'이다 !!
        char[][] strBox = new char[5][15];

        for (int i = 0; i < 5; i++) {
            String str = br.readLine();
            for (int j = 0; j < str.length(); j++) {
                strBox[i][j] = str.charAt(j);
            }
        }

        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 5; j++) {
                if (strBox[j][i] == ' ' || strBox[j][i] == '\0') {
                    continue;
                }

                System.out.print(strBox[j][i]);
            }
        }
    }
}
